...
jest.mock("next/server", () => ({
  NextResponse: {
    ...
    redirect: (url: URL | string) => ({
      status: 307,
      headers: {
        get: (k: string) => (k === "Location" ? url.toString() : null),
      },
    }),
    next: () => ({
      headers: { set: jest.fn() },
...
describe("Parent-only financial access", () => {
  const { requireParent } = require("@/lib/api-auth");

  it("allows parents", () => {
    expect(requireParent("parent")).toBeNull();
  });

  it.each(["teen", "child", undefined])(
    "rejects non-parent role %s",
    (role) => {
      const result = requireParent(role);
      expect(result).not.toBeNull();
      expect(result.status).toBe(403);
    },
  );
