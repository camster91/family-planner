    ...
    await prisma.$transaction(
      async (tx) => {
        ...
        for (const source of plan.recipeIngredients) {
          ...
          const recipeId = existing("Recipe", source.sourceRecipeId),
        ...
        for (const source of plan.mealPlans) {
          ...
          const target = await tx.mealPlan.create({
            data: {
              family_id: options.familyId,
              name: source.name,
          ...
          await track("MealPlan", source.sourceId, "MealPlan", target.id);
          increment(summary.created, "MealPlan");
        }
        for (const source of plan.mealPlanEntries) {
          ...
          const mealPlanId = existing("MealPlan", source.sourceMealPlanId),
            recipeId = existing("Recipe", source.sourceRecipeId);
          ...
          const target = await tx.mealPlanEntry.upsert({
            where: {
              meal_plan_id_date_meal_type: {
          ...
          increment(summary.created, "MealPlanEntry");
        }
        for (const source of plan.shoppingLists) {
          ...
          const target = await tx.shoppingList.create({
            data: {
              family_id: options.familyId,
              name: source.name,
              created_by: source.createdBy,
          ...
          increment(summary.created, "ShoppingList");
        }
        for (const source of plan.shoppingItems) {
          ...
          const shoppingListId = existing(
            "ShoppingList",
            source.sourceShoppingListId,
          );
          if (!shoppingListId) {
            skip(
              "ShoppingItem",
          ...
          const target = await tx.shoppingItem.create({
            data: {
              shopping_list_id: shoppingListId,
              ingredient_name: source.ingredientName,
              amount: source.amount,
              unit: source.unit,
              category: source.category,
              checked: source.checked,
              recipe_id: source.sourceRecipeId
                ? (existing("Recipe", source.sourceRecipeId) ?? null)
                : null,
            },
          });
          await track(
          ...
          increment(summary.created, "ShoppingItem");
        }
