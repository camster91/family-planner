...
import { isFamilyImportSource, runFamilyImport } from '@/lib/imports/run-family-import'
...
export async function POST(
  ...
  if (!isFamilyImportSource(source)) {
    return NextResponse.json({ error: 'Unsupported import source' }, { status: 404 })
  }

  const declaredSize = Number(request.headers.get('content-length') ?? 0)
  if (declaredSize > MAX_IMPORT_BYTES) {
    return NextResponse.json({ error: 'Import payload exceeds 10 MB' }, { status: 413 })
  }
  ...
  if (!parsed.success) {
    return NextResponse.json({ error: 'Invalid import request', details: parsed.error.flatten() }, { status: 400 })
  }
  ...
  try {
    ...
    if (error instanceof z.ZodError) {
      return NextResponse.json({ error: 'Invalid source export', details: error.flatten() }, { status: 400 })
    }
