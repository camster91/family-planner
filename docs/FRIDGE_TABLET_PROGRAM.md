...
## 31. Implementation status notes

### Today board vertical slice (#119 / #159), 2026-09-26

- **Route:** `/dashboard/today`, inside the existing dashboard auth/layout. `?mode=fridge` hides the app chrome (top bar and tab bar are removed from layout, focus order and the accessibility tree) for a mounted tablet, asks for a screen wake lock (best effort) and shows an "Exit fridge mode" action. The legacy `/dashboard` home is unchanged, so this is revertible by removing the route.
- **Regions:** Today (date, clock, still-to-come events including imported calendar events labelled "From <calendar>"), Dinner tonight (`FamilyMeal`, `meal_type = 'dinner'`), Groceries (`src/lib/shopping-snapshot.ts`), Chores today per person (date-only helpers in `src/lib/dates.ts`), Coming up (next 3 local days with events and planned dinners). Every region has an explicit empty state; meal planning turned off has its own state.
- **Shared-surface DTO:** `src/app/dashboard/today/today-board-data.ts` selects only approved fields per query, scoped by `family_id`. It never reads budget, allowance, messages, medications, sick days, locations/addresses, handoff, account settings or XP/streak data, and it leaves out event `location`/`description` and meal `notes` (free text that can carry addresses or private notes). Private data is not sent to the page, rather than hidden client-side, so the same DTO can back a shared-device session when #157 lands.
- **Roles:** the route is on the kid allowlist (`src/lib/kid-access.ts`); the content is shared household data every member may already read (ROLE_AND_ISOLATION_MATRIX.md). Links to calendar, chores, meals and features are offered only to roles that may open them.
- **Local day:** the server returns a zone-agnostic window; the client picks "today" in the viewer's zone (`src/components/fridge/board-model.ts`), so the board renders after mount with a short skeleton.
- **Refresh:** client-side only (`router.refresh()` every 5 minutes while visible and online, on reconnect, and on return to a visible tab with old data). No server job. Offline and stale states say when the data was last updated.
- **Brand:** no XP, points, streaks or leaderboards; no overdue shaming (the board shows chores due today only).
- **Evidence:** unit tests in `src/app/dashboard/today/__tests__/` and `src/components/fridge/__tests__/`; `e2e/fridge.spec.ts` (regions, empty household, child role, privacy canaries checked against the rendered HTML and RSC payload, fridge mode, 44px targets, offline, client refresh, axe, visual baselines at 1280×800 and 800×1280).
- **Not yet:** Figma spec (#131), weather, use-soon/inventory (#158), grocery quick add, chore completion from the board, assistant entry point, night mode, shared-device sessions (#157). In fridge mode the layout still serializes the signed-in person's nav props (name, email) into the page even though the nav is hidden; a device session or a chrome-free layout should remove that.
