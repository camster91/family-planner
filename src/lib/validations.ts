...
export const registerSchema = z
  ...
  .superRefine((data, context) => {
    if (data.role !== "parent" && !data.inviteCode) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["inviteCode"],
        message:
          "A parent-controlled family invitation is required for child and teen accounts",
      });
    }
  });
...
export const createEventSchema = z.object({
  ...
  event_type: z
    .enum(["school", "sports", "appointment", "family", "work", "other"])
    .default("other"),
});
...
export const createListSchema = z.object({
  ...
  description: z
    .union([z.string().max(500).trim(), z.null()])
    .optional()
...
export const updateUserSchema = z.object({
  name: z.string().min(1).max(100).trim().optional(),
  age: z
    .union([z.number().int().min(1).max(150), z.string(), z.null()])
    .optional(),
});
