...
describe("planChoreChampsImport", () => {
  ...
  it("rejects malformed dates before any database mutation is planned", () => {
    ...
    expect(() =>
      planChoreChampsImport(invalid, { "kid-1": "user-1" }),
    ).toThrow();
  });
  ...
  it("plans habits, rewards, goals, and badges without importing source identities", () => {
    ...
    expect(plan.habitLogs[0]).toMatchObject({
      sourceHabitId: "habit-1",
      targetUserId: "user-1",
    });
    expect(plan.rewards[0]).toMatchObject({ name: "Movie", cost: 20 });
    expect(plan.redemptions[0]).toMatchObject({
      sourceRewardId: "reward-1",
      targetUserId: "user-1",
    });
    expect(plan.familyGoals[0]).toMatchObject({
      targetPoints: 100,
      currentPoints: 45,
    });
    expect(plan.badges[0].requirement).toEqual({
      type: "legacy",
      value: "FIRST_CHORE",
    });
    expect(plan.earnedBadges[0]).toMatchObject({
      sourceBadgeId: "badge-1",
      targetUserId: "user-1",
    });
