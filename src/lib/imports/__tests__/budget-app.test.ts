...
describe("planBudgetAppImport", () => {
  it("converts cents to dollars and preserves advanced finance records", () => {
    ...
    expect(plan.wishlistItems[0]).toMatchObject({
      title: "Bike",
      approxPrice: 299.99,
    });
  ...
  it("defaults to a database-free dry run", async () => {
    const result = await importBudgetApp(source, {
      familyId: "family-1",
      startedBy: "user-1",
    });
