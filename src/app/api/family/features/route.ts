...
export async function PATCH(request: Request) {
  ...
  if (user.role !== "parent") {
    return NextResponse.json(
      { error: "Only parents can change features" },
      { status: 403 },
    );
  }
  if (!user.family_id) {
  ...
  // Shape B: { key, enabled } — single feature (kept for backwards compat + future API)
    ...
    return NextResponse.json(
      { error: "features object OR (key + enabled) required" },
      { status: 400 },
    );
  }

  const changed = Object.keys(next).filter(
    (key) => current[key as FeatureKey] !== next[key as FeatureKey],
  );
  await prisma!.$transaction([
    ...
    prisma!.auditLog.create({
      data: {
        ...
        actor_id: user.id,
        action: "family.features_updated",
        resource_type: "Family",
        resource_id: user.family_id,
        metadata: { changed },
      },
    }),
  ]);
