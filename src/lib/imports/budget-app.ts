...
export const budgetAppExportSchema = z.object({
  ...
  transactions: z
    .array(
      z.object({
        ...
        duplicateOfId: z.string().nullish(),
  ...
  wishlistItems: z
    ...
    .default([]),
  accounts: z.array(archiveRecord).default([]),
  bills: z.array(archiveRecord).default([]),
  billPayments: z.array(archiveRecord).default([]),
  budgets: z.array(archiveRecord).default([]),
  goals: z.array(archiveRecord).default([]),
  goalContributions: z.array(archiveRecord).default([]),
  incomes: z.array(archiveRecord).default([]),
  screenshotReceipts: z.array(archiveRecord).default([]),
  spendingPatterns: z.array(archiveRecord).default([]),
  dailyPeriods: z.array(archiveRecord).default([]),
...
export function planBudgetAppImport(input: unknown, importedBy: string) {
  ...
  const transactions = source.transactions.map((item) => {
    ...
    if (item.categoryId && !categoryId)
      skippedRecords.push({
        ...
        sourceId: item.id,
        reason: `Missing source category ${item.categoryId}`,
      });
    return {
      sourceId: item.id,
      userId: importedBy,
  ...
  const archive = archiveFields.flatMap((field) =>
  ...
  return {
    ...
    categories: source.categories.map((item) => ({
      ...
      createdAt: item.createdAt,
      metadata: {
        dailyCapCents: item.dailyCap,
        sourceParentId: item.parentId,
        rules: item.rules,
      },
    })),
    transactions,
    wishlistItems: source.wishlistItems.map((item) => ({
      sourceId: item.id,
      requestedBy: importedBy,
      title: item.name.trim(),
      link: item.link || null,
      approxPrice: item.price / 100,
      status: item.purchased ? "received" : "idle",
      description: `Imported priority: ${item.priority}`,
      createdAt: item.createdAt,
    })),
    archive,
    skippedRecords,
