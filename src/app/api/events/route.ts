...
export async function POST(request: NextRequest) {
  try {
    ...
    const { title, description, start_time, end_time, location, event_type } =
      parsed.data;
    ...
    await Promise.all(
      recipients.map((recipient) =>
        notificationServiceServer.sendNotification({
          ...
          message: `${auth.user.name} added "${title}" to the calendar`,
          type: "event",
        }),
      ),
    );
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (updates.start_time !== undefined)
      data.start_time = new Date(updates.start_time);
    if (updates.end_time !== undefined)
      data.end_time = new Date(updates.end_time);
